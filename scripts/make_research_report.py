...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--results-json",
        type=Path,
        default=Path("results/summary/nhanes_hdbscan_results_v2_plus.json"),
        help="Path to aggregate NHANES-HDBSCAN results JSON.",
    )
    return parser.parse_args()


def main() -> int:
    """Run the aggregate reporting workflow."""
    args = parse_args()
    cfg = ResearchConfig(results_json=args.results_json)
